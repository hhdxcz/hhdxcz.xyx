...
function main() {
  ...
  const matcher = createMatchmaker({ gameId });
  ...
  const onJoin = () => {
    ...
    showResult({
      title: "已加入房间",
      code,
      hint: "把房间号发给朋友，或在其他设备上使用同一房间号加入。",
    });
    enterRoom(code);
  };
  ...
  if (matchBtn) {
    matchBtn.addEventListener("click", async () => {
      if (matching) {
        matching = false;
        matcher.cancel();
        if (matchBtn) matchBtn.textContent = "匹配模式";
        setBusy(false);
        showResult({ title: "已取消匹配", code: "----", hint: "你可以重新点击匹配模式，或创建/加入房间。" });
        return;
      }

      matching = true;
      if (matchBtn) matchBtn.textContent = "取消匹配";
      setBusy(true);
      if (matchBtn) matchBtn.disabled = false;
      showResult({
        title: "匹配中…",
        code: "----",
        hint: `正在等待其他人同时匹配（本地演示：同一浏览器打开另一个标签页也点匹配）。`,
      });
      try {
        const res = await matcher.start();
        const code = normalizeRoomCode(res.room);
        matching = false;
        if (matchBtn) matchBtn.textContent = "匹配模式";
        const preferred = clientId < String(res.otherId || "") ? clientId : String(res.otherId || "");
        showResult({ title: "已匹配到房间", code, hint: "你们已匹配到同一个房间号，等待房主开始。" });
        enterRoom(code, { preferredHostId: preferred });
      } catch {
        matching = false;
        if (matchBtn) matchBtn.textContent = "匹配模式";
      } finally {
  ...
  if (createBtn) {
    createBtn.addEventListener("click", () => {
      ...
      showResult({ title: "已创建房间", code, hint: "等待别人输入房间号加入，房主点击开始后才会进入游戏。" });
